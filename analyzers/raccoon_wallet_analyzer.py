...
def _wallet_chat_id() -> str:
    chat_id = (os.getenv("TELEGRAM_CHAT_ID_RACCOON_WALLET") or "").strip()
    if not chat_id:
...
def analyze_raccoon_wallets(payin_path: str, payout_path: str | None = None):
    ...
    try:
        ...
        send_message_sync(f"⚠️ Не удалось прочитать PayIn: {e}", chat_id=_wallet_chat_id())
        return
    ...
    if not bad:
        send_message_sync("🟢 Все партнёры в норме!", chat_id=_wallet_chat_id())
        return
    ...
    send_message_sync("\n".join(lines), chat_id=_wallet_chat_id())
