...
def send_file(path: str, caption: str | None = None):
    ...
    if not os.path.exists(path):
        logger.warning(f"⚠️ Файл для отправки не найден: {path}")
        return
    try:
        send_file_sync(path, caption, CHAT_ID)
        time.sleep(3)  # даём очереди Telegram время на обработку
    except Exception as e:
        logger.error(f"❌ Ошибка при отправке файла в Telegram: {e}")

# -----------------------------
# Нормализация и загрузка данных
# -----------------------------
