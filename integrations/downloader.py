...
# === Безопасная обёртка для Telegram ===
def send(text: str):
    """Отправляет сообщение в Telegram, если CHAT_ID задан."""
    if not CHAT_ID:
...
def _upload_local_to_dropbox(local_path: str, dropbox_name: str) -> str:
    """Заливает локальный файл в Dropbox и возвращает имя."""
...
def _ensure_logged_in(page, context):
    """Авторизация в Antares или использование сохранённой сессии."""
...
def _download_wallet_export(page, timestamp: str) -> str:
    """Скачивает wallet-экспорт (card.xlsx)."""
    ...
    try:
        ...
        logger.exception(f"❌ Ошибка при экспорте wallet: {e}")
        raise


def _download_payin_export(page, timestamp: str) -> str:
    """Скачивает PayIn-экспорт (conversion.xlsx)."""
...
def _download_extra_files(page, timestamp: str) -> list[str]:
    """Скачивает дополнительные файлы (cd, payout)."""
    local_paths = []
    tz = zoneinfo.ZoneInfo("Europe/Moscow")
    ...
    try:
        ...
        try:
            ...
            if any(s.startswith("cd_") for s in uploaded_extra) and any(s.startswith("payout_") for s in uploaded_extra):
                ...
                send(f"🚀 Запущен анализ Payout ({payout_name})")
            else:
                logger.info("ℹ️ Файлы cd_/payout_ не найдены — Payout пропущен.")
                send("ℹ️ Файлы cd_/payout_ не найдены — Payout пропущен.")
        finally:
            release_lock()
            send("✅ Downloader завершил цикл успешно.")
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        send("🚀 Downloader запущен вручную на Railway")
        run_download()
    except Exception as e:
